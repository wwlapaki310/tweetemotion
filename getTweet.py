...
import datetime, time, sys, io, re
import pandas as pd
...
if __name__ == '__main__':
    ...
    for tweet in getter.collect(total = -1):

        # 本文中の改行削除
        text = re.sub(r'\n', " ", tweet['text'])

        tmp = pd.DataFrame([tweet['created_at'], tweet['user']['screen_name'], text], index=["date", "user", "tweet"])
        res = res.append(tmp.T)

        #print ('{}\t{}\t{}'.format(tweet['created_at'], '@'+tweet['user']['screen_name'], text))

    res.set_index(["date"],inplace=True)

    res.to_csv(args.uname+".txt", sep='\t', encoding="utf-8")
    #savepklfile(res, obj+".pkl")
